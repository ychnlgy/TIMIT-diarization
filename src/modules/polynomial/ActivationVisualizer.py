...
class ActivationVisualizer(torch.nn.Module):
    ...
    Stores the inputs it recieves.

    This is used to produce the input histograms and activation visualizations
    in the paper. Parameters are:

        - k: number of input features to visualize
        - module: torch.nn.Module. For example, LinkActivation or torch.nn.ReLU.

    Typical use would be to use these to wrap around existing modules in the
    model, then calling visualize to save the images, then calling reset to
    clear memory.
